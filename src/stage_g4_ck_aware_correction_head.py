"""
Stage G4: CK-aware correction head for CIKD++-RT no_c_emb.
Implements the wrapper model, dry-run checks, shape checks, checkpoint loading, forward pass, and training path reachability checks.
"""
...
from models.cikd_pp_rt import TVCSSpecialist, ResidualTransformerFusion, CIKDPPResidualTransformer
...
class CIKDPPCKCorrectionModel(nn.Module):
    """
    CIKDPPCKCorrectionModel wraps CIKDPPResidualTransformer base model and adds
    a small CK-aware correction head to resolve confusion between class 2 (CK) and class 3.
    """
    def __init__(self, base_model, correction_scale=0.5, dropout=0.2):
        super().__init__()
        ...
        # 5. proj_text (256)
        # 6. base_probs (6)
        # 7. abs(proj_kg - proj_zv) (256)
        # 8. proj_kg * proj_zv (256)
        # Total dimension: 512 + 6 * 256 + 6 = 2054
        self.feature_dim = tvcs_dim + 6 * d_model + num_classes
    ...
    def forward(self, text_features, image_global_features, image_patch_features, kg_features, relation_ids, baseline_logits):
        # Run base model with ablation_no_c_emb=True (preserving no_c_emb architecture)
        ...
        base_final_logits = base_outputs['logits_final']
        z_v = base_outputs['z_v']  # [B, tvcs_dim]
        
        # Extract relation embedding and project features using the base model's projection layers
        relation_embedding = self.base_model.tvcs_specialist.relation_embed(relation_ids)
        
        proj_text = self.base_model.residual_transformer.proj_text(text_features)
        proj_img = self.base_model.residual_transformer.proj_img(image_global_features)
        proj_kg = self.base_model.residual_transformer.proj_kg(kg_features)
        proj_rel = self.base_model.residual_transformer.proj_rel(relation_embedding)
        proj_zv = self.base_model.residual_transformer.proj_zv(z_v)
        
        # Base probabilities
        base_probs = torch.softmax(base_final_logits, dim=-1)
        
        # Compute absolute difference and product
        diff_kg_zv = torch.abs(proj_kg - proj_zv)
        prod_kg_zv = proj_kg * proj_zv
        ...
        # Compute gate and raw delta
        ck_gate = torch.sigmoid(self.gate_mlp(features))
        ck_delta_raw = self.delta_mlp(features)
        
        # Build a 6-way delta vector initialized to zeros using differentiable torch.cat
        col_0 = torch.zeros(base_final_logits.shape[0], 1, device=base_final_logits.device)
        col_1 = torch.zeros(base_final_logits.shape[0], 1, device=base_final_logits.device)
        col_2 = ck_delta_raw[:, 0:1]
        col_3 = ck_delta_raw[:, 1:2]
        col_4 = torch.zeros(base_final_logits.shape[0], 1, device=base_final_logits.device)
        col_5 = torch.zeros(base_final_logits.shape[0], 1, device=base_final_logits.device)
        
        correction_delta = torch.cat([col_0, col_1, col_2, col_3, col_4, col_5], dim=-1)
        
        # final_logits = base_final_logits + correction_scale * ck_gate * correction_delta
        final_logits = base_final_logits + self.correction_scale * ck_gate * correction_delta
        
        return {
            "logits_final": final_logits,
            "logits_base": base_final_logits,
            "logits_delta": base_outputs['logits_delta'],
            "correction_delta": correction_delta,
            "ck_gate": ck_gate,
            "ck_delta_raw": ck_delta_raw,
            "alpha": base_outputs['alpha'],
            "c_logit": base_outputs['c_logit'],
            "z_v": z_v,
            "attention": base_outputs['attention']
        }

def check_promotion_gate(val_macro_f1, val_ck_f1, val_score, tvcs_auc):
    metric_ok = (val_macro_f1 > 0.4800) or (val_ck_f1 > 0.4000) or (val_score > 0.5000)
...
def run_dry_run(args):
    ...
    if not os.path.exists(args.init_checkpoint):
        print(f"[-] Init checkpoint missing: {args.init_checkpoint}")
        sys.exit(1)

    # Overwrite check for report path only in dry-run
    report_path = os.path.join(args.out_dir, "G4_DRY_RUN_REPORT.txt")
    ...
    print("\n" + "=" * 80)
    print(f"Stage G4 Dry-Run Checks: {args.config_name}")
    print("=" * 80)
    ...
    base_model = CIKDPPResidualTransformer(
    ...
    # Load TVCS Specialist checkpoint into base model
    ...
    base_model.tvcs_specialist.load_state_dict(tvcs_state)
    print("  [+] TVCS Specialist loaded successfully.")

    # Load init checkpoint into base model
    ...
    first_key = list(init_state.keys())[0]
    if first_key.startswith('base_model.'):
        # Clean prefix if it is wrapped
        clean_state = {k.replace('base_model.', ''): v for k, v in init_state.items()}
    ...
    # Instantiate correction model wrapper
    print(f"Instantiating CIKDPPCKCorrectionModel for config '{args.config_name}'...")
    model = CIKDPPCKCorrectionModel(
    ...
    unfrozen_names = []
    for name, param in model.base_model.tvcs_specialist.named_parameters():
    ...
    base_final_logits = outputs['logits_base']
    logits_delta = outputs['logits_delta']
    correction_delta = outputs['correction_delta']
    ck_gate = outputs['ck_gate']
    ck_delta_raw = outputs['ck_delta_raw']
    alpha = outputs['alpha']
    c_logit = outputs['c_logit']

    print(f"[+] Output Shapes and Values Checked:")
    print(f"    base_final_logits shape: {list(base_final_logits.shape)} (Expected: [4, 6])")
    print(f"    correction_delta shape:  {list(correction_delta.shape)} (Expected: [4, 6])")
    print(f"    final_logits shape:       {list(final_logits.shape)} (Expected: [4, 6])")
    print(f"    ck_gate shape:           {list(ck_gate.shape)} (Expected: [4, 1])")
    print(f"    ck_gate values:          {ck_gate.squeeze(-1).cpu().numpy().tolist()}")
    print(f"    correction values for class 2: {correction_delta[:, 2].cpu().numpy().tolist()}")
    print(f"    correction values for class 3: {correction_delta[:, 3].cpu().numpy().tolist()}")
    print(f"    alpha value:             {alpha.item():.4f}")

    assert list(final_logits.shape) == [4, 6], f"[-] final_logits shape mismatch: {final_logits.shape}"
    assert list(base_final_logits.shape) == [4, 6], f"[-] base_final_logits shape mismatch: {base_final_logits.shape}"
    assert list(correction_delta.shape) == [4, 6], f"[-] correction_delta shape mismatch: {correction_delta.shape}"
    ...
    # L2 regularization for the correction delta raw values
    loss_corr_reg = args.correction_l2_weight * torch.mean(ck_delta_raw ** 2)

    dummy_loss = loss_cls + args.lambda_tvcs * loss_tvcs + args.residual_mu * loss_res + loss_corr_reg
    ...
    # Write G4_DRY_RUN_REPORT.txt
    os.makedirs(args.out_dir, exist_ok=True)
    with open(report_path, 'w') as f:
        f.write(f"G4 DRY-RUN REPORT: {args.config_name}\n")
        f.write("=======================================\n\n")
        ...
        f.write(f"Correction Scale:    {args.correction_scale}\n\n")
        ...
        f.write(f"  Init checkpoint loaded: True\n")
        f.write(f"  Trainable parameters:   {trainable_params}\n")
        f.write(f"  Frozen parameters:      {frozen_params}\n\n")
        f.write("Forward Pass Check:\n")
        f.write(f"  base_final_logits shape: {list(base_final_logits.shape)}\n")
        f.write(f"  correction_delta shape:  {list(correction_delta.shape)}\n")
        f.write(f"  final_logits shape:      {list(final_logits.shape)}\n")
        f.write(f"  ck_gate shape:           {list(ck_gate.shape)}\n")
        f.write(f"  ck_gate values:          {ck_gate.squeeze(-1).cpu().numpy().tolist()}\n")
        f.write(f"  correction class 2 values: {correction_delta[:, 2].cpu().numpy().tolist()}\n")
        f.write(f"  correction class 3 values: {correction_delta[:, 3].cpu().numpy().tolist()}\n")
...
def run_training_train_val_only(args):
    ...
    planned_outputs = [
        args.checkpoint_out,
        os.path.join(args.out_dir, 'G4_TRAINING_LOG.csv'),
        os.path.join(args.out_dir, 'G4_BEST_VAL_METRICS.csv'),
        os.path.join(args.out_dir, 'G4_TVCS_VAL_METRICS.csv'),
        os.path.join(args.out_dir, 'G4_PER_CLASS_F1_VAL.csv'),
        os.path.join(args.out_dir, 'G4_CONFUSION_MATRIX_VAL.csv'),
        os.path.join(args.out_dir, 'G4_CK_CLASS3_ERROR_TRACE.csv'),
        os.path.join(args.out_dir, 'G4_TRAINING_SUMMARY.txt')
    ]
    ...
    print("\nStarting G4 training execution...")
    os.makedirs(args.out_dir, exist_ok=True)
    ...
    if sample_ids is not None:
        val_sample_ids = torch.tensor(sample_ids[val_mask], dtype=torch.long)
    else:
        val_sample_ids = torch.tensor(np.where(val_mask)[0], dtype=torch.long)

    # Dataloaders
    train_ds = TensorDataset(tr_text, tr_img_g, tr_img_p, tr_kg, tr_rel, tr_lbl, tr_y_ck, tr_logits)
    val_ds = TensorDataset(val_text, val_img_g, val_img_p, val_kg, val_rel, val_lbl, val_y_ck, val_logits, val_sample_ids)
    ...
    # Instantiate correction wrapper model
    model = CIKDPPCKCorrectionModel(
        base_model=base_model,
        correction_scale=args.correction_scale,
        dropout=args.dropout
    ...
    for epoch in range(args.epochs):
        ...
        for bx_text, bx_img_g, bx_img_p, bx_kg, bx_rel, bx_lbl, bx_y_ck, bx_logits in train_loader:
            ...
            loss_total = loss_cls + args.lambda_tvcs * loss_tvcs + args.residual_mu * loss_res + loss_corr_reg
        ...
        val_gates, val_deltas, val_base_logits, val_final_logits = [], [], [], []
        val_sample_ids_list = []
        
        with torch.no_grad():
            for bx_text, bx_img_g, bx_img_p, bx_kg, bx_rel, bx_lbl, bx_y_ck, bx_logits, bx_sids in val_loader:
                bx_text = bx_text.to(device)
                bx_img_g = bx_img_g.to(device)
                bx_img_p = bx_img_p.to(device)
                bx_kg = bx_kg.to(device)
                ...
                val_gates.extend(ck_gate.cpu().numpy())
                val_deltas.extend(ck_delta_raw.cpu().numpy())
                val_base_logits.extend(base_final_logits.cpu().numpy())
                val_final_logits.extend(logits_final.cpu().numpy())
                val_sample_ids_list.extend(bx_sids.numpy())

        val_preds = np.array(val_preds)
        val_targets = np.array(val_targets)
        val_c_probs = np.array(val_c_probs)
        val_y_ck_arr = np.array(val_y_ck_list)
        val_gates = np.array(val_gates)
        val_deltas = np.array(val_deltas)
        val_base_logits = np.array(val_base_logits)
        val_final_logits = np.array(val_final_logits)
        ...
        # Confusion Count of class 2 predicted as class 3
        ck_as_class3_count = int(np.sum((val_targets == 2) & (val_preds == 3)))
        class3_as_ck_count = int(np.sum((val_targets == 3) & (val_preds == 2)))
        avg_ck_gate = float(np.mean(val_gates))

        # Validation Selection Score
        val_score = 0.45 * macro_f1 + 0.35 * ck_f1 + 0.20 * tvcs_auc

        print(f"Epoch {epoch+1:02d}/{args.epochs:02d} | Loss: {train_loss:.4f} | Val Acc: {acc:.4f} | Val Macro-F1: {macro_f1:.4f} | Val CK-F1: {ck_f1:.4f} | Val TVCS AUC: {tvcs_auc:.4f} | Gate Avg: {avg_ck_gate:.4f} | Conf CK->3: {ck_as_class3_count} | Score: {val_score:.4f}")
        ...
        history.append({
            ...
            'val_ck_f1': ck_f1,
            'val_class3_f1': class3_f1,
            'val_tvcs_auc': tvcs_auc,
            'val_selection_score': val_score,
            'avg_ck_gate': avg_ck_gate,
            'ck_as_class3_count': ck_as_class3_count,
            'class3_as_ck_count': class3_as_ck_count
        })
        ...
        if val_score > best_score:
            ...
            checkpoint = {
                ...
                'val_metrics': {
                    ...
                    'sample_ids': val_sample_ids_arr.tolist(),
                    'gates': val_gates.tolist(),
                    'deltas': val_deltas.tolist(),
                    'base_logits': val_base_logits.tolist(),
                    'final_logits': val_final_logits.tolist()
                }
            }
            torch.save(checkpoint, args.checkpoint_out)
            print(f"  [+] Saved best checkpoint with Val Score {val_score:.4f} (Gate Avg: {avg_ck_gate:.4f})")
        else:
            patience_counter += 1
    ...
    val_sample_ids_arr = np.array(best_m['sample_ids'])
    val_gates = np.array(best_m['gates'])
    val_deltas = np.array(best_m['deltas'])
    val_base_logits = np.array(best_m['base_logits'])
    val_final_logits = np.array(best_m['final_logits'])

    # 1. G4_TRAINING_LOG.csv
    pd.DataFrame(history).to_csv(os.path.join(args.out_dir, 'G4_TRAINING_LOG.csv'), index=False)
    
    # 2. G4_BEST_VAL_METRICS.csv
    df_best = pd.DataFrame([{
        ...
        'ck_f1': best_m['ck_f1'],
        'class3_f1': best_m['class3_f1'],
        'ck_as_class3_count': best_m['ck_as_class3_count'],
        'class3_as_ck_count': best_m['class3_as_ck_count'],
        'tvcs_auc': best_m['tvcs_auc_ck_vs_real'],
        'selection_score': checkpoint['val_score'],
        'best_epoch': checkpoint['epoch'],
        'avg_ck_gate': best_m['avg_ck_gate']
    }])
    df_best.to_csv(os.path.join(args.out_dir, 'G4_BEST_VAL_METRICS.csv'), index=False)

    # 3. G4_TVCS_VAL_METRICS.csv
    ...
    df_tvcs.to_csv(os.path.join(args.out_dir, 'G4_TVCS_VAL_METRICS.csv'), index=False)

    # 4. G4_PER_CLASS_F1_VAL.csv
    df_per_class = pd.DataFrame({
        'class_id': list(range(6)),
        'f1_score': best_m['per_class_f1']
    })
    df_per_class.to_csv(os.path.join(args.out_dir, 'G4_PER_CLASS_F1_VAL.csv'), index=False)

    # 5. G4_CONFUSION_MATRIX_VAL.csv
    ...
    df_cm.to_csv(os.path.join(args.out_dir, 'G4_CONFUSION_MATRIX_VAL.csv'), index=True)
    ...
    try:
        ...
        ax.set(xticks=np.arange(cm.shape[1]),
               ...
               title=f'G4 {args.config_name} Val CM',
               ylabel='True label',
        ...
        fig.tight_layout()
        plt.savefig(os.path.join(args.out_dir, 'G4_CONFUSION_MATRIX_VAL.png'), dpi=150)
        plt.close()
    except Exception as e:
        print(f"[-] Could not plot CM PNG: {e}")

    # 6. G4_CK_CLASS3_ERROR_TRACE.csv
    # Extract validation samples that are true class 2 but predicted as class 3, or vice-versa
    error_mask = ((val_targets == 2) & (val_preds == 3)) | ((val_targets == 3) & (val_preds == 2))
    error_indices = np.where(error_mask)[0]
    
    error_trace_rows = []
    for idx in error_indices:
        b_probs = np.exp(val_base_logits[idx]) / np.sum(np.exp(val_base_logits[idx]))
        f_probs = np.exp(val_final_logits[idx]) / np.sum(np.exp(val_final_logits[idx]))
        error_trace_rows.append({
            'sample_id': int(val_sample_ids_arr[idx]),
            'true_label': int(val_targets[idx]),
            'predicted_label': int(val_preds[idx]),
            'ck_gate_val': float(val_gates[idx][0]),
            'ck_delta_class2': float(val_deltas[idx][0]),
            'ck_delta_class3': float(val_deltas[idx][1]),
            'base_prob_class2': float(b_probs[2]),
            'base_prob_class3': float(b_probs[3]),
            'final_prob_class2': float(f_probs[2]),
            'final_prob_class3': float(f_probs[3])
        })
    pd.DataFrame(error_trace_rows).to_csv(os.path.join(args.out_dir, 'G4_CK_CLASS3_ERROR_TRACE.csv'), index=False)

    # Check promotion gate
    promoted = check_promotion_gate(best_m['macro_f1'], best_m['ck_f1'], checkpoint['val_score'], best_m['tvcs_auc_ck_vs_real'])

    # 7. G4_TRAINING_SUMMARY.txt
    summary_path = os.path.join(args.out_dir, 'G4_TRAINING_SUMMARY.txt')
    with open(summary_path, 'w') as f:
        f.write(f"Stage G4 Training Summary: {args.config_name}\n")
        f.write("===============================================\n")
        ...
        f.write(f"Val Class 3-F1:          {best_m['class3_f1']:.4f}\n")
        ...
        f.write(f"Avg CK Gate:             {best_m['avg_ck_gate']:.6f}\n")
        f.write(f"CK predicted as Class 3: {best_m['ck_as_class3_count']}\n")
        f.write(f"Class 3 predicted as CK: {best_m['class3_as_ck_count']}\n")
        ...
        f.write(f"  Val CK-F1 > 0.4000:        {best_m['ck_f1'] > 0.4000} ({best_m['ck_f1']:.4f})\n")
...
def run_evaluation_only(args):
    ...
    print("\nStarting G4 locked test evaluation execution...")
    os.makedirs(args.out_dir, exist_ok=True)
    ...
    # Wrap model with correction head
    model = CIKDPPCKCorrectionModel(
        base_model=base_model,
        correction_scale=args.correction_scale,
        dropout=args.dropout
    ...
    test_gates, test_deltas, test_base_logits, test_final_logits = [], [], [], []
    ...
    with torch.no_grad():
        for bx_text, bx_img_g, bx_img_p, bx_kg, bx_rel, bx_lbl, bx_y_ck, bx_logits, bx_sid in test_loader:
            ...
            test_gates.extend(ck_gate.cpu().numpy())
            test_deltas.extend(ck_delta_raw.cpu().numpy())
            test_base_logits.extend(base_final_logits.cpu().numpy())
            test_final_logits.extend(logits_final.cpu().numpy())
    ...
    test_gates = np.array(test_gates)
    test_deltas = np.array(test_deltas)
    test_base_logits = np.array(test_base_logits)
    test_final_logits = np.array(test_final_logits)
    ...
    # Confusion Counts
    ck_as_class3_count = int(np.sum((test_targets == 2) & (test_preds == 3)))
    class3_as_ck_count = int(np.sum((test_targets == 3) & (test_preds == 2)))
    avg_ck_gate = float(np.mean(test_gates))

    print(f"\nLocked Test Results | Accuracy: {acc:.4f} | Macro-F1: {macro_f1:.4f} | Weighted-F1: {weighted_f1:.4f} | CK-F1: {ck_f1:.4f} | Class3-F1: {class3_f1:.4f} | TVCS AUC: {tvcs_auc:.4f}")
    print(f"Mean C Real: {mean_c_real:.4f} | Mean C CK: {mean_c_ck:.4f} | TVCS Delta: {tvcs_delta:.4f} | Avg CK Gate: {avg_ck_gate:.4f} | CK->3 Count: {ck_as_class3_count}")

    # Required Outputs:
    # 1. G4_D_LOCKED_TEST_METRICS.csv
    metrics_path = os.path.join(args.out_dir, 'G4_D_LOCKED_TEST_METRICS.csv')
    ...
    # 2. G4_D_LOCKED_TEST_PER_CLASS_F1.csv
    per_class_path = os.path.join(args.out_dir, 'G4_D_LOCKED_TEST_PER_CLASS_F1.csv')
    df_per_class = pd.DataFrame({
        'class_id': list(range(6)),
    ...
    # 3. G4_D_LOCKED_TEST_TVCS_METRICS.csv
    tvcs_path = os.path.join(args.out_dir, 'G4_D_LOCKED_TEST_TVCS_METRICS.csv')
    df_tvcs = pd.DataFrame([{
        'config': args.config_name,
        'tvcs_auc_ck_vs_real': tvcs_auc,
        'tvcs_pr_auc': tvcs_pr_auc,
    ...
    # 4. G4_D_LOCKED_TEST_CONFUSION_MATRIX.csv
    ...
    cm_path_csv = os.path.join(args.out_dir, 'G4_D_LOCKED_TEST_CONFUSION_MATRIX.csv')
    df_cm.to_csv(cm_path_csv, index=True)
    print(f"[+] Saved confusion matrix CSV to {cm_path_csv}")

    # Plot confusion matrix PNG
    cm_path_png = os.path.join(args.out_dir, 'G4_D_LOCKED_TEST_CONFUSION_MATRIX.png')
    try:
        import matplotlib.pyplot as plt
        fig, ax = plt.subplots(figsize=(6, 6))
        im = ax.imshow(cm, interpolation='nearest', cmap=plt.cm.Blues)
        ...
        ax.set(xticks=np.arange(cm.shape[1]),
               ...
               xticklabels=[f'Pred {i}' for i in range(6)],
               yticklabels=[f'True {i}' for i in range(6)],
               title=f'G4 {args.config_name} Locked Test Confusion Matrix',
               ylabel='True label',
               xlabel='Predicted label')
    ...
    # 5. G4_D_LOCKED_TEST_PREDICTIONS.csv
    preds_path = os.path.join(args.out_dir, 'G4_D_LOCKED_TEST_PREDICTIONS.csv')
    df_preds = pd.DataFrame({
        ...
        'y_ck': test_y_ck_arr,
        'ck_gate': test_gates[:, 0]
    })
    df_preds.to_csv(preds_path, index=False)
    print(f"[+] Saved predictions CSV to {preds_path}")

    # 6. G4_D_LOCKED_TEST_SUMMARY.txt
    summary_path = os.path.join(args.out_dir, 'G4_D_LOCKED_TEST_SUMMARY.txt')
    with open(summary_path, 'w') as f:
        f.write(f"Stage G4 Locked Test Summary: {args.config_name}\n")
        f.write("=================================================================\n")
        ...
        f.write(f"  Class3-F1: {class3_f1:.6f}\n")
        ...
        f.write(f"  ck_as_class3_count: {ck_as_class3_count}\n")
        f.write(f"  class3_as_ck_count: {class3_as_ck_count}\n")
        f.write(f"  avg_ck_gate: {avg_ck_gate:.6f}\n")
        f.write(f"  Per-Class F1: {per_class_f1.tolist()}\n")
    print(f"[+] Saved locked test summary to {summary_path}")
    print("=" * 80)

def main():
    parser = argparse.ArgumentParser(description="Stage G4: CK-aware correction head for CIKD++-RT")
    ...
    parser.add_argument('--correction_scale', type=float, default=0.5, help='correction_scale')
    ...
    parser.add_argument('--correction_l2_weight', type=float, default=1e-4, help='L2 regularization weight for correction delta values')
    ...
    if args.eval_only:
        if args.out_dir is None or args.checkpoint_out is None:
            parser.error("--out_dir and --checkpoint_out are required for evaluation.")
